...
int ErrorsCheck::ErrorChecking()
{
	...
	/*switch (Balans(i, LEOF, LStartBlock, LEndBlock))
	{
	case 1:
		Err_num++;
	...
	/*switch (Balans(0, LEOF, LStartBlock, LEndBlock))
	{
	case 1:
		Err_num++;
		fprintf(ef, "line %d: \tToo much '%s'!\n", Data->LexTable[j].line, Data->LexName[LEndBlock].name.c_str());
		break;
	case 2:
		Err_num++;
		fprintf(ef, "line %d: \t'%s' expected!\n", Data->LexTable[j].line, Data->LexName[LEndBlock].name.c_str());
	...
	for (j = 0;; j++)
	{
		...
		if (Data->LexTable[j].type == LFor)				//перевірка for Ci
		{
			if (Data->LexTable[j + 1].type != LLBraket)
			{
				Err_num++;
				fprintf(ef, "line %d: \t'(' expected after '%s'!\n", Data->LexTable[j + 1].line, Data->LexName[LFor].name.c_str());
			}
			else if (Data->LexTable[j + 2].type != LIdentifier)
			{
				Err_num++;
				fprintf(ef, "line %d: \tIdentifier expected after '%s'!\n", Data->LexTable[j + 1].line, Data->LexName[LFor].name.c_str());
			}
			else if (Data->LexTable[j + 3].type != LNewValue)
			{
				Err_num++;
				fprintf(ef, "line %d: \t'%s' expected after identifier!\n", Data->LexTable[j + 2].line, Data->LexName[LNewValue].name.c_str());
			}
			else if (IsExpression(j + 4, ef) != 0)
			{
				Err_num++;
				fprintf(ef, "line %d: \tIdentifier or number expected after '%s'!\n", Data->LexTable[j + 3].line, Data->LexName[LNewValue].name.c_str());
			}
			while ((Data->LexTable[j].type != LSeparator) && (Data->LexTable[j].type != LEOF)) j++;

			Err_num += IsExpression((j + 1), ef);
			j++;
			while ((Data->LexTable[j].type != LSeparator) && (Data->LexTable[j].type != LEOF)) j++;
			if (Data->LexTable[j + 1].type != LIdentifier)
			{
			...
			else if (Data->LexTable[j + 2].type != LNewValue)
			{
				Err_num++;
				fprintf(ef, "line %d: \t '%s' expected after identifier!\n", Data->LexTable[j + 3].line, Data->LexName[LNewValue].name.c_str());
			}
			...
			j++;
			while ((Data->LexTable[j].type != LStart) && (Data->LexTable[j].type != LEOF)) j++;
			if (Data->LexTable[j - 1].type != LRBraket)
			{
				Err_num++;
				fprintf(ef, "line %d: \t')' expected!\n", Data->LexTable[j].line);
			}
