	...
	for (;; ++i)
	{
		...
		if (l.type == LEnd && Data->LexTable[i + 1].type != LEOF)
		{
			if (opStack.back())
			{
				...
				GenASMCode(Data->LexTable[temp + 1].name, f);
		...
		{
			int nv = i;
			while (Data->LexTable[nv].type != LNewValue)
			{
				nv++;
			}
			...
			GenASMCode(Data->LexTable[nv - 1].name, f);
			...
			fputs("\tcmp eax, 1\n", f);
			fprintf(f, "\tjne forFinish%d\n", loopLabelIndex);
			while (Data->LexTable[i].type != LNewValue)
			{
				i++;
			}
			...
			while (Data->LexTable[i].type != LStart)
			{
				i++;
			}
			i--;
		...
		if (l.type == LNewValue && Data->LexTable[i - 3].type != LFor)
		{
			int bufi;
			bufi = i;
